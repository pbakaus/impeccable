...
Setup (context gathering, register) is already loaded by then; sub-commands don't re-invoke `/skill:impeccable`.
...
**Pin** creates a standalone shortcut so `/skill:<command>` invokes `/skill:impeccable <command>` directly. **Unpin** removes it. The script writes to every harness directory present in the project.
...
`/skill:impeccable hooks <on|off|status|ignore-rule|ignore-file|ignore-value|reset>` manages the design detector hook for this project. The hook auto-runs the detector after direct UI file edits and surfaces findings as system reminders. Full flow is in [reference/hooks.md](reference/hooks.md); load it when the user invokes `/skill:impeccable hooks` with any argument.
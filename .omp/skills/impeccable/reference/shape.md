...
**Output**: A design brief that can be handed off to /skill:impeccable craft, or directly to /skill:impeccable for freeform implementation. When visual direction probes are used, the images are supporting artifacts, not the primary output.
...
Once confirmed, the brief is complete. The user can now hand it to /skill:impeccable, or use it to guide any other implementation approach. (If the user wants the full discovery-then-build flow in one step, they should use /skill:impeccable craft instead, which runs this command internally.)

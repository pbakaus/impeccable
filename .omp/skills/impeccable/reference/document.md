...
- The user just ran `/skill:impeccable init` and needs the visual side documented.
...
Decide by scanning first (Scan mode Step 1). If the scan finds no tokens, no component files, and no rendered site, offer seed mode; don't silently switch. `/skill:impeccable document --seed` forces seed mode regardless of code presence.
...
Before interviewing: "There's no existing visual system to scan. I'll ask five quick questions to seed a starter DESIGN.md. You can re-run `/skill:impeccable document` once there's code, to capture the real tokens and components. OK?"
...
```markdown
<!-- SEED: re-run /skill:impeccable document once there's code to capture the actual tokens and components. -->
```

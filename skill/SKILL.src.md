...
2. If the user invoked a sub-command (`craft`, `shape`, `audit`, `polish`, ...), you MUST read `reference/<command>.md` next. Non-optional. The reference defines the command's flow; without it you will skip steps the user expects. **When the project platform is native** (`ios` / `android` / `adaptive`, per the `context.mjs` directive) **and the Commands table lists a native variant for the command, read the variant instead** (e.g. `reference/audit.native.md`).

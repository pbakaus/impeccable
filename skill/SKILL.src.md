...
5. Read the matching platform reference **when the project targets native mobile.** If PRODUCT.md's `## Platform` field is `ios` or `android`, also read `reference/<platform>.md` for native conventions (HIG / Material). If it is `adaptive` (a cross-platform app shipping both, e.g. Flutter / React Native), read **both** `reference/ios.md` and `reference/android.md`. This is on top of the register reference, not a replacement. If the field is `web`, absent, or legacy, there is nothing extra to read; the General rules already assume web. `context.mjs` prints the exact platform directive when one applies.

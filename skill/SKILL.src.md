...
**Launcher unavailable (degraded Setup):** On refusal or failure, send a separate message **before the next tool call**: “Context loading did not run; I’ll read the existing project context directly.” Then read existing PRODUCT.md and DESIGN.md without inventing missing context, follow applicable steps 2–3, and continue through permitted tools. A degraded run still owes every must-read: the command's reference and [reference/craft-floor.md](reference/craft-floor.md) before any UI edit, and [reference/document.md](reference/document.md) before writing DESIGN.md. This applies to planning and editing; launcher failure alone does not block either. <!-- rule:skill-setup-degraded-pack -->

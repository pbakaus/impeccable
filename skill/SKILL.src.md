...
1. Run `node {{scripts_path}}/context.mjs "<url-or-file-path>"` once per session, passing the URL or file path you're working on (the argument is optional). If the project keeps more than one design system — a DESIGN.md inside named subdirectories of the context dir (e.g. `admin/`, `marketing/`) — the target routes to the matching scope's DESIGN.md; otherwise the shared DESIGN.md is used. If you've already seen its output in this conversation, do not re-run it. The script either prints the project's PRODUCT.md (and the relevant DESIGN.md when present) as a markdown block, or tells you it's missing. Follow whatever it prints. **If it reports `NO_PRODUCT_MD`, stop and follow `reference/init.md` before doing anything else.** If the output ends with an `UPDATE_AVAILABLE` directive, follow it (ask the user once about updating, then continue). It never blocks the current task.

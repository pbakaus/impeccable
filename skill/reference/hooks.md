...
Plain HTML (`.html`, `.htm`) uses the DOM engine. Components and templates (`.vue`, `.svelte`, `.astro`, `.blade.php`) retain the full text scan and add DOM checks in both the hook and `impeccable detect`. Text findings keep their line numbers, cache identities, and Stop attribution. DOM style checks require locally resolvable styles; unresolved classes or preprocessor style blocks defer computed-style checks to a scan of the running site. Cursor scans proposed contents with the real file path so linked stylesheets resolve from the project.

Add other markup stacks under **`detector.extensions`**, for example `{ "ext": ".html.erb", "engine": "html" }`. These templates get the same combined scan, including full-page copy analyzers. Suffix matching supports double extensions; longest suffix wins and configuration overrides a built-in on ties. `engine` defaults to `html`; `text` selects only the text pipeline. Automatic directory and hook scans include built-ins plus configured HTML suffixes. Other configured text suffixes require an explicit `detect <file>` target. Directory scans skip `vendor` dependency trees.

...
Server-side template files (Blade, Twig, ERB, Handlebars) are outside the built-in extension list, so the hook skips them until the project declares them under **`detector.extensions`**: one entry per extension, `{ "ext": ".blade.php", "engine": "html" }`. `engine` picks the analyzer (`html` for markup templates, `text` for JS/TS/CSS-like files) and defaults to `html`. Matching runs against the end of the filename, so double extensions like `.blade.php` and `.html.erb` work. Config only adds extensions; the built-in list always applies.

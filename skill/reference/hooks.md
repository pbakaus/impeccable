...
Built-in markup (`.html`, `.htm`, `.vue`, `.svelte`, `.astro`, `.blade.php`) is scanned with the DOM engine by both the hook and `impeccable detect`. Add other template stacks under **`detector.extensions`** when the project uses Twig, ERB, Handlebars, or HEEx; those sit outside the built-in list and are skipped until declared. One entry per extension, `{ "ext": ".html.erb", "engine": "html" }`. `engine` picks the analyzer (`html` for markup templates, `text` for JS/TS/CSS-like files) and defaults to `html`. Match against the end of the filename, so double extensions like `.html.erb` work. Longest suffix wins. A configured entry for a built-in suffix overrides the default engine. Config only adds or overrides extensions; the built-in list always applies.

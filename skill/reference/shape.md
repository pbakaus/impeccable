...
**Output**: A chat-confirmed design brief that can be handed off to {{command_prefix}}impeccable craft, or directly to {{command_prefix}}impeccable for freeform implementation. When visual direction probes are used, the images are supporting artifacts, not the primary output.
...
If PRODUCT.md, BRAND.md, or DESIGN.md is missing, stop and run {{command_prefix}}impeccable init first. Shape should not create those project-level files.
...
Once confirmed, the shape run is complete. The user can now hand the brief to {{command_prefix}}impeccable, or use it to guide any other implementation approach. (If the user wants the full discovery-then-build flow in one step, they should use {{command_prefix}}impeccable craft instead, which runs this command internally.)

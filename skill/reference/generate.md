...
**Reply and wait in one call.** In one-shot polling, the done reply takes `--then-poll`, so the reply and the wait for the user's choice are the same command; run it the way your harness runs a long wait (**Claude Code** in the foreground with your tool's longest timeout, 600000 ms, so you are paused until the choice arrives; **Codex** in a yielded foreground exec; **Cursor** in a background terminal with notify on `"type":"(accept|discard|variant_mount_failed|exit)"`), and never pass a short `--timeout=`. The next event arrives as that call's output, with the reply's ack folded in as `_replyAck`. While it runs there is nothing else to do: never sleep and never poll its output on a timer; a harness that backgrounds it wakes you when it returns. `{"type":"timeout"}` means the user has not chosen yet: run `live-poll` again and keep waiting. In stream mode, reply as live.md says.
...
Done when `live-complete` printed `phase: "completed"` and the stop output reported `Removed live script tag from <file>.` Nothing else is owed: `live-complete` already verified the source, and the dev server keeps serving the accepted design without a restart, so skip leftover scans, curls, and detector runs here.

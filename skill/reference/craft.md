...
Run {{command_prefix}}impeccable shape with whatever feature description the user provided. Shape is **required** for craft; it produces the task-specific confirmed direction. It does not replace the project-level init files.

If PRODUCT.md, BRAND.md, or DESIGN.md is missing and the task depends on brand/site direction, stop and run {{command_prefix}}impeccable init first.

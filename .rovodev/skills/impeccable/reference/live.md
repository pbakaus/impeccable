...
```bash
node .rovodev/skills/impeccable/scripts/live.mjs [--target <path>]
```
...
```
LOOP:
  node .rovodev/skills/impeccable/scripts/live-poll.mjs [--target <path>]   # default long timeout; no --timeout=

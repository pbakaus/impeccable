...
```bash
node .rovodev/skills/impeccable/scripts/live-wrap.mjs --id EVENT_ID --count EVENT_COUNT --element-id "ELEMENT_ID" --classes "class1,class2" --tag "div" --text "TEXT_SNIPPET" --page-url "/path"
```
...
```bash
# every entry applied
node .rovodev/skills/impeccable/scripts/live-poll.mjs --reply EVENT_ID done --data '{"status":"done","appliedEntryIds":["8hexid"],"failed":[],"files":["src/page.html"],"notes":[]}'
```

```bash
# some entries failed (still apply the rest)
node .rovodev/skills/impeccable/scripts/live-poll.mjs --reply EVENT_ID done --data '{"status":"partial","appliedEntryIds":["8hexid"],"failed":[{"entryId":"9f0e1d2c","reason":"originalText not found","candidates":[]}],"files":["src/page.html"],"notes":[]}'
```

```bash
# no entry could be applied
node .rovodev/skills/impeccable/scripts/live-poll.mjs --reply EVENT_ID done --data '{"status":"error","appliedEntryIds":[],"failed":[{"entryId":"9f0e1d2c","reason":"could not resolve sources"}],"files":[],"notes":[],"message":"could not resolve sources for any entry"}'
```
...
```bash
node .rovodev/skills/impeccable/scripts/live-poll.mjs --reply EVENT_ID done --data '{"status":"applied","entries":3}'
```
...
```bash
node .rovodev/skills/impeccable/scripts/live-poll.mjs --reply done --file src/page.html
```
...
- To retry an Apply from chat (only when the user explicitly asks and no Track A event is already leased), run `node .rovodev/skills/impeccable/scripts/live-commit-manual-edits.mjs --page-url=/<route>` directly. The script prints the same JSON shape as the endpoint and the buffer / rollback semantics are identical.

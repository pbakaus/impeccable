...
If the user asks which PRODUCT.md or DESIGN.md Impeccable loads for a file, route, or app path, treat that as a diagnostic request, not a design task. `--target` must be a real file or directory path; map routes and app names to source paths first. Run `node .claude/skills/impeccable/scripts/context.mjs --target <path>`, report the resolved `productPath`, `designPath`, `projectRoot`, and `repoRoot` from the command's `RESOLVED_CONTEXT` output, and stop. Your final answer must name the command you ran, including `--target <path>`. Do not answer from a directory listing, opened files, or by manually walking ancestor folders; the resolver owns monorepo inheritance and overrides. A correct-looking answer without running the resolver is wrong; if you cannot run the command, say that instead of guessing.

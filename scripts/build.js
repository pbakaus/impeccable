...
import { hooksJsonFor, buildClaudePluginHooksManifest, buildCursorPluginHooksManifest } from './lib/transformers/hooks.js';
...
async function build() {
  ...
  if (BUILD_OPTIONS.syncRootOutputs) {
    ...
    // Build the native Cursor plugin subtree at ./plugin-cursor/.
    //
    // Cursor installs plugins from a directory containing a
    // `.cursor-plugin/plugin.json` manifest, auto-discovering components from
    // `skills/`, `agents/`, and `hooks/hooks.json`. This mirrors the Claude
    // plugin path above but targets Cursor's manifest + hook conventions:
    //   - hooks use Cursor's `preToolUse` blocking gate (not Claude's
    //     `PostToolUse`), resolved relative to the plugin root.
    //   - the manifest is generated from the same root `.claude-plugin/plugin.json`
    //     source of truth, so versions never drift (guarded in
    //     validate-plugin-versions.js).
    //   - the repo-root `.cursor-plugin/marketplace.json` registers it for
    //     Cursor marketplace submission with `source: "./plugin-cursor"`.
    const cursorPluginRoot = path.join(ROOT_DIR, 'plugin-cursor');
    const cursorPluginManifestDir = path.join(cursorPluginRoot, '.cursor-plugin');
    const cursorPluginSkillsDir = path.join(cursorPluginRoot, 'skills');
    const cursorPluginAgentsDir = path.join(cursorPluginRoot, 'agents');
    const cursorPluginHooksDir = path.join(cursorPluginRoot, 'hooks');
    if (fs.existsSync(cursorPluginManifestDir)) fs.rmSync(cursorPluginManifestDir, { recursive: true });
    if (fs.existsSync(cursorPluginSkillsDir)) fs.rmSync(cursorPluginSkillsDir, { recursive: true });
    if (fs.existsSync(cursorPluginAgentsDir)) fs.rmSync(cursorPluginAgentsDir, { recursive: true });
    if (fs.existsSync(cursorPluginHooksDir)) fs.rmSync(cursorPluginHooksDir, { recursive: true });

    // Cursor plugin manifest, derived from the root plugin.json source of truth.
    // Drop the Claude-specific `skills` path override (Cursor auto-discovers
    // `skills/`) and add the fields Cursor's manifest documents.
    ...
    fs.mkdirSync(cursorPluginManifestDir, { recursive: true });
    fs.writeFileSync(
      path.join(cursorPluginManifestDir, 'plugin.json'),
      JSON.stringify(cursorPluginManifest, null, 2) + '\n',
    );

    // Plugin README (Cursor marketplace submissions should document usage).
    // Generated so it never drifts from the manifest version.
    const cursorPluginReadme = [
    ...
    fs.writeFileSync(path.join(cursorPluginRoot, 'README.md'), cursorPluginReadme);

    // Skill payload: reuse the generated Cursor skill (same one shipped to
    // `.cursor/skills/`), so the plugin carries the Cursor-flavored SKILL.md,
    // references, scripts, and bundled detector.
    const cursorSkillsSrc = path.join(DIST_DIR, 'cursor', '.cursor', 'skills', 'impeccable');
    if (fs.existsSync(cursorSkillsSrc)) {
      fs.mkdirSync(cursorPluginSkillsDir, { recursive: true });
      copyDirSync(cursorSkillsSrc, path.join(cursorPluginSkillsDir, 'impeccable'));
    }

    // Agents: Cursor reads `agents/*.md` with the same name/description
    // frontmatter Claude uses, so reuse the generated Claude agent files. Only
    // harness-agnostic agents land here (the Codex-only asset-producer is
    // already excluded from the Claude agent build via its `providers` field).
    if (fs.existsSync(claudeAgentsSrc)) {
      copyDirSync(claudeAgentsSrc, cursorPluginAgentsDir);
    }

    // Ship the design detector as a plugin-packaged Cursor hook. Cursor
    // auto-discovers `hooks/hooks.json` at the plugin root and runs its command
    // from there, so the pre-write blocking gate resolves against the bundled
    // skill payload rather than a project `.cursor/skills/` layout.
    fs.mkdirSync(cursorPluginHooksDir, { recursive: true });
    fs.writeFileSync(
      path.join(cursorPluginHooksDir, 'hooks.json'),
      JSON.stringify(buildCursorPluginHooksManifest(), null, 2) + '\n',
    );

    // Repo-root Cursor marketplace manifest (multi-plugin form). Generated from
    // the same source of truth so its version tracks the root plugin.json.
    const rootCursorMarketplaceDir = path.join(ROOT_DIR, '.cursor-plugin');
    fs.mkdirSync(rootCursorMarketplaceDir, { recursive: true });
    ...
    fs.writeFileSync(
      path.join(rootCursorMarketplaceDir, 'marketplace.json'),
      JSON.stringify(cursorMarketplace, null, 2) + '\n',
    );

    console.log('📦 Built Cursor plugin subtree at ./plugin-cursor/');
  } else {

...
```bash
node .pi/skills/impeccable/scripts/live.mjs [--target <path>]
```
...
```
LOOP:
  node .pi/skills/impeccable/scripts/live-poll.mjs [--target <path>]   # default long timeout; no --timeout=

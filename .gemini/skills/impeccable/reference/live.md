...
```bash
node .gemini/skills/impeccable/scripts/live.mjs [--target <path>]
```
...
```
LOOP:
  node .gemini/skills/impeccable/scripts/live-poll.mjs [--target <path>]   # default long timeout; no --timeout=

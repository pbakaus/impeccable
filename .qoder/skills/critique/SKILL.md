...
Auto-select 2–3 personas most relevant to this interface type (use the selection table in the reference). If `QODER.md` contains a `## Design Context` section from `teach-impeccable`, also generate 1–2 project-specific personas from the audience/brand info.
...
**After presenting findings**, use targeted questions based on what was actually found. ask the user directly in a concise way if clarification is needed. These answers will shape the action plan.

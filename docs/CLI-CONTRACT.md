...
- Tests: `crates/context/src/generate_image.rs` (mock API request/sidecar regression tests), `tests/image-generation-eval.mjs` (opt-in billed model comparison), `tests/new-work-e2e.test.mjs` (fake mode chain, opt-in `test:new-work-e2e`).

...
3. For each target, in order:
   ...
   - `detectLocalFile(fp, opts)`: `uses_html_engine(fp, detector.extensions)` → `detectHtml(fp, opts)`; else `detectText(readFileSync(fp,'utf-8'), fp, opts)`. Built-in HTML-engine suffixes (matched with `endsWith` on the basename, not last-segment `extname`): `.html`, `.htm`, `.vue`, `.svelte`, `.astro`, `.blade.php`. Configured `detector.extensions` entries overlay that set; longest suffix wins; a configured entry of equal length is authoritative (`engine: "html"` → DOM, `engine: "text"` → text). Directory walks also include configured suffixes so `detect .` covers `.html.erb` once declared.

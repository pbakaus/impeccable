...
#### `generate-image` -> `{{scripts_path}}/impeccable generate-image`

- **Invoked from**: `context_cli.rs append_image_gen_directive` (only when `OPENAI_API_KEY` is set) advertises `impeccable generate-image` with the selected default model and billing disclosure. `new-work.md` and `visualize.md` use the helper for comps and reference images; `visualize.md` records approval on its prompt sidecar.
- **Args**: `--prompt <text>` or `--prompt-file <path>` (file wins), `--out <path>` (required), `--size <WxH>` (default `1536x1024`), `--quality` (default `medium`; the 2.5 models also accept `xhigh` and `max`), `--ref <img>` repeatable, `--model <id>` (default `gpt-image-2.5-flare`; use `gpt-image-2.5-sunburst` for precision editing or `gpt-image-2` for a baseline comparison), `--background transparent|opaque|auto` (optional; omitted preserves the API default). An explicit background adds `background` and `output_format=png` to either API payload and `background` / `outputFormat: "png"` to the sidecar. Missing/invalid background values exit 1 before network or output; every explicit background requires a `.png` path (case-insensitive), otherwise exit 1. Transparent PNG bytes, including partial alpha, are preserved through prompt embedding.
- **Fake mode** `IMPECCABLE_IMAGE_GEN_FAKE=1`: no network; `.svg` out → SVG with prompt-hashed 2-3 color gradient (FNV-1a), wrapped prompt text, `SYNTHETIC COMP` label; otherwise a minimal PNG (RGB palette stripes by default; `--background transparent` produces RGBA stripes with clear padding, `tEXt` chunk `Comment\0SYNTHETIC COMP: <prompt>`); size parsed by `/^(\d+)x(\d+)$/` else 1536x1024; `stdout> IMAGE: ${out} (${w}x${h}, fake synthetic comp, $0.00, no API call)`; exit 0. Missing prompt/out → stderr `generate-image: --prompt (or --prompt-file) and --out are required.` exit 1.
- **Real mode**: The selected model is used for both endpoints, the sidecar, and stdout; payload examples below show the default. No `OPENAI_API_KEY` → stderr `generate-image: OPENAI_API_KEY is not set; use the harness-native image tool instead.` exit 1. Without refs: `POST https://api.openai.com/v1/images/generations`, headers `Authorization: Bearer <key>`, `content-type: application/json`, body `{"model":"gpt-image-2.5-flare","prompt":...,"size":...,"quality":...,"n":1}`. With refs: `POST https://api.openai.com/v1/images/edits` multipart FormData fields `model=gpt-image-2.5-flare`, `prompt`, `size`, `quality`, `n=1`, `image[]` blobs (type png/webp else jpeg, filename basename). Non-ok → stderr `generate-image: API error ${status}: ${first 300 chars}` exit 1; no `data[0].b64_json` → `generate-image: no image in response` exit 1. Writes decoded bytes to `--out`; best-effort in-process `embed-prompt <out> --prompt <prompt>` and sidecar `${out}.json` = `{prompt, createdAt: ISO, tool: 'impeccable generate-image', model: '<selected model>', [refs]}` (2-space). stdout `IMAGE: ${out} (${size}, ${quality}, ${model}, billed to your OpenAI key); prompt embedded + sidecar at ${out}.json`; exit 0.
- Tests: `crates/context/src/generate_image.rs` (mock API request/sidecar, RGBA preservation, background validation and fake-alpha regression tests), `tests/image-generation-eval.mjs` (opt-in billed model comparison), `tests/new-work-e2e.test.mjs` (fake mode chain, opt-in `test:new-work-e2e`).

#### comp-fidelity verbs: `comp-spec` / `comp-diff` / `font-match` / `build-phase`

Ported from the former `skill/scripts/{comp-spec,comp-diff,font-match,build-phase}.mjs` (+ `lib/{png,raster,image-metrics,font-fingerprint,font-index,hero-checks}.mjs`) into the engine; invoked as `{{scripts_path}}/impeccable <verb>`. All four resolve paths against the process cwd. Printed commands spell the launcher via `IMPECCABLE_SELF` (default `impeccable`; existing launcher file paths with shell-sensitive characters are quoted, while command prefixes such as `npx impeccable` are preserved), so they name `{{scripts_path}}/impeccable <verb>`, never `node …mjs`. ISO `createdAt`/`startedAt` timestamps in stdout and written JSON are the only run-dependent output.

- **`comp-spec`** — turns an approved comp into a measured build spec. `--comp <png> --grid` writes `.impeccable/build/comp-grid.png` (10x10 labeled grid) and prints PALETTE/BANDS/NEXT; `--comp <png> --regions <json>` measures regions into `.impeccable/build/spec.json` (region box, sampled palette, medium, aspect, detail energy, plate path for raster kinds) and prints the spec; `--comp <png> --auto` derives band regions; `--print` prints the compact spec; `--crop <id> [--out f] [--scale n] [--raw]` writes a reference crop; `--plate-prompt <id> [--background transparent|opaque|auto]` prints the regeneration prompt. Transparent prompts preserve clear margins and object placement, white foreground paint, fine edges, and interior gaps instead of asking for edge-to-edge fill; opaque/auto/omitted preserve the existing prompt. Invalid or missing explicit background values exit 1. This option only authors the prompt: also pass `--background transparent` to `generate-image` when using the API fallback. `--spec <path>` overrides the spec path (default `.impeccable/build/spec.json`). Validation refusals (stderr, exit 1) are the JS strings verbatim: a region with no id / duplicate id / no note, a code-kind region whose note names painted material, a code region over 25% of the comp, a grid span that is not `<colrow>:<colrow>`, uncovered ink cells without `allowUncovered`. spec.json is byte-identical to the JS output.

...
```bash
node .kiro/skills/impeccable/scripts/live.mjs [--target <path>]
```
...
```
LOOP:
  node .kiro/skills/impeccable/scripts/live-poll.mjs [--target <path>]   # default long timeout; no --timeout=

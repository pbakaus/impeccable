...
1. Run `node <skill-base-dir>/scripts/context.mjs` once per session (`<skill-base-dir>` is this skill's loaded base directory, shown by the runtime when it loads the skill; keep cwd at the user's project). Pass a named source file or route as `--target <path>`. It loads PRODUCT.md, DESIGN.md, the matching surface brief, and native-platform guidance when applicable; follow its directives and do not rerun it.

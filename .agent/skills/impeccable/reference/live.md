...
```
node .agent/skills/impeccable/scripts/live-poll.mjs --stream   # stays running; one JSON line per event
...
```bash
node .agent/skills/impeccable/scripts/live-status.mjs
node .agent/skills/impeccable/scripts/live-resume.mjs --id SESSION_ID
node .agent/skills/impeccable/scripts/live-complete.mjs --id SESSION_ID
```
...
```bash
node .agent/skills/impeccable/scripts/live-poll.mjs --reply EVENT_ID error "Short reason"
```
...
```bash
node .agent/skills/impeccable/scripts/live-poll.mjs --reply EVENT_ID steer_done ["Optional short note for a browser toast"]
```

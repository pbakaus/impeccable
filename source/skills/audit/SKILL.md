...
## Save Recommendations as Plan

**After generating the report**, write a `.impeccable-plan.md` file in the project root so that `{{command_prefix}}next` and `{{command_prefix}}fix-all` can execute the recommendations automatically.

Format:
```markdown
# Impeccable Plan
Generated by: {{command_prefix}}audit on [today's date]
Target: [the area that was audited, or "entire project"]

## Pending
1. `{{command_prefix}}command-name` — Brief description of what to fix (specific context from audit findings)
...
- Combine related issues into a single command entry (e.g., all theming issues → one `{{command_prefix}}normalize` entry)
...
> Saved N recommendations to `.impeccable-plan.md`. Run `{{command_prefix}}next` to execute them one at a time, or `{{command_prefix}}fix-all` to run them all.

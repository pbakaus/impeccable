...
**After generating the critique**, write a `.impeccable-plan.md` file in the project root so that `{{command_prefix}}next` and `{{command_prefix}}fix-all` can execute the recommendations automatically.
...
- Order by priority — most impactful design improvements first

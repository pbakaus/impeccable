...
1. Run `node .agents/skills/impeccable/scripts/context.mjs` once per session. In a monorepo, `--target <path>` must be a real file or directory path. If the user names a route or child app, map it to the matching source file or app directory first, then pass that path so project-scoped PRODUCT.md / DESIGN.md files can override root defaults. A user asking which PRODUCT.md or DESIGN.md was loaded for a specific path is making a context diagnostic request: run `node .agents/skills/impeccable/scripts/context.mjs --target <path>` and answer from its `RESOLVED_CONTEXT` fields; do not infer this from the file tree, opened files, or memory. If you accidentally ran `context.mjs` without `--target` for a path-specific monorepo request, that output is not valid evidence; immediately rerun with `--target <path>` before answering. If you've already seen its output for the same target in this conversation, do not re-run it. The script either prints the project's PRODUCT.md (and DESIGN.md when present) as a markdown block, or tells you it's missing. Follow whatever it prints. **If it reports `NO_PRODUCT_MD`, stop and follow `reference/init.md` before doing anything else.** If the output ends with an `UPDATE_AVAILABLE` directive, follow it (ask the user once about updating, then continue). It never blocks the current task.

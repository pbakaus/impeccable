...
use crate::color::{
    color_to_hex, contrast_ratio, get_hue, has_chroma, is_gray_ink, is_neutral_color,
...
pub fn check_colors(opts: &ColorOpts) -> Vec<RuleHit> {
    ...
    if opts.has_direct_text && opts.text_color.is_some() && !opts.is_emoji_only {
        ...
        if let Some(bgs) = bgs {
            // Gray is low chroma at whatever lightness the ink sits at, and
            // the surface is a colour when it has chroma of its own. The old
            // pair of tests read relative luminance as if it were lightness,
            // which made every off-white under 0.85 gray and charged an
            // off-white nav on a teal masthead three times over (REN-404).

...
pub fn collect_browser_findings(dom: &dyn Dom, config: &BrowserConfig) -> CollectResult {
    ...
    for el in dom.query_all(None, "*").unwrap_or_default() {
        ...
        if !scoped_ignore_active(dom, el, "ai-color-palette")
            && waiving_selector(&config.ignore_selectors, "ai-color-palette", |sel| {
                matches!(dom.closest(el, sel), Ok(Some(_)))
            }).is_none()
        {
...
mod tests {
    ...
    fn ignored_colors_do_not_contribute_tell_hues() {
        ...
        let charged = |d: &FakeDom, config: &BrowserConfig| -> Vec<String> {
            collect_browser_findings(d, config)
                .groups
                .iter()
                .flat_map(|g| g.findings.iter().filter(|f| f.ignored_by.is_none()).map(|f| f.type_.clone()))
                .filter(|t| t == "ai-color-palette")
                .collect()
        };
        // Two tell hues, neither waived: the palette is the page's.
        assert_eq!(charged(&build(false), &BrowserConfig::default()).len(), 2);
        // The cyan half waived: one tell hue is an accent, and the purple ink
        // outside the ignored subtree is not charged either.
        assert!(charged(&build(true), &BrowserConfig::default()).is_empty());
        // Config waivers have the same boundary as the attribute: waived
        // components cannot turn an unrelated accent into a palette finding.
        for rule in ["ai-color-palette", "*"] {
            let config = BrowserConfig {
                ignore_selectors: vec![SelectorIgnore::new(rule, ".demo")],
                ..Default::default()
            };
            assert!(charged(&build(false), &config).is_empty());
        }
    }

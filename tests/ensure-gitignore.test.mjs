...
describe('ensure-gitignore — CLI', () => {
  ...
  it('default mode writes the block and reports changed + tracked artifacts', () => {
    makeGitRepo(tmp);
    // pre-create a shared artifact so it shows up in `tracked`
  ...
  it('--check reports present + tracked without writing', () => {
    makeGitRepo(tmp);
    writeFileSync(join(tmp, '.gitignore'), 'node_modules\n');
...
describe('ensure-gitignore — pattern surface', () => {
  it('covers the screenshots and runtime paths observed in real projects', () => {
    ...
    // the big pollution sources from safivo/web

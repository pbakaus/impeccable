...
describe('detectHtml — static HTML/CSS fixtures', () => {
  ...
  it('color: APCA keeps branded orange action text and catches WCAG dark-mode false passes', async () => {
    const f = await detectHtml(path.join(FIXTURES, 'color.html'));
    ...
    assert.equal(
      ...
      false,
      'white action text on Uplinked orange should pass APCA even though WCAG 2 reports 2.7:1'
    );
    ...
    assert.ok(
      apcaDarkModeFinding,
      'gray body text on black should fail APCA even though WCAG 2 reports a high ratio'
    );
  });

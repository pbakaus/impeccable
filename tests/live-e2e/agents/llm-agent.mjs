...
export const MANUAL_EDIT_SYSTEM_INSTRUCTIONS = [
  ...
  '- For JSX numeric display expressions, replace the whole expression with a quoted JSX expression. Example sourceEdit: originalText="{String(item.count)}", newText="{\\"7 seats\\"}". Do not replace item.count or change count: 7.',
  '- For Svelte numeric display expressions, replace the whole template expression with a quoted string expression. Example sourceEdit: originalText="{item.count}", newText="{\\"7 seats\\"}". Do not replace item.count or change count: 7.',

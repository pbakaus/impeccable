...
```bash
node .opencode/skills/impeccable/scripts/live.mjs [--target <path>]
```
...
```
LOOP:
  node .opencode/skills/impeccable/scripts/live-poll.mjs [--target <path>]   # default long timeout; no --timeout=

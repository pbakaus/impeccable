...
```bash
node .opencode/skills/impeccable/scripts/live-wrap.mjs --id EVENT_ID --count EVENT_COUNT --element-id "ELEMENT_ID" --classes "class1,class2" --tag "div" --text "TEXT_SNIPPET" --page-url "/path"
```
...
- `--page-url` ← `event.pageUrl`. **Required when pending manual edits may affect the picked source block** (`live-wrap.mjs` will exit with `missing_page_url_with_pending_edits` for that case). Scopes the buffer-aware "original" content step to edits made on this page so the wrap block's `data-impeccable-variant="original"` reflects the user's staged DOM, not the un-edited source. Pass it every call: the buffer state can change between events, and forgetting it can produce variants that look correct in source but ignore the user's pending changes.
...
If an `originalText` is not found in `sourceHint` or any usable candidate, mark that entry failed and move to the next entry. Do not fail solely because `sourceHint` is missing when candidate evidence identifies source data. Do not partially apply the remaining ops for that failed entry, do not retry blindly, do not fuzzy-match, and do not create a new file. If you already changed another op in the same entry before discovering the failure, undo that change before replying.
...
```bash
# every entry applied
node .opencode/skills/impeccable/scripts/live-poll.mjs --reply EVENT_ID done --data '{"status":"done","appliedEntryIds":["8hexid"],"failed":[],"files":["src/page.html"],"notes":[]}'
```

```bash
# some entries failed (still apply the rest)
node .opencode/skills/impeccable/scripts/live-poll.mjs --reply EVENT_ID done --data '{"status":"partial","appliedEntryIds":["8hexid"],"failed":[{"entryId":"9f0e1d2c","reason":"originalText not found","candidates":[]}],"files":["src/page.html"],"notes":[]}'
```

```bash
# no entry could be applied
node .opencode/skills/impeccable/scripts/live-poll.mjs --reply EVENT_ID done --data '{"status":"error","appliedEntryIds":[],"failed":[{"entryId":"9f0e1d2c","reason":"could not resolve sources"}],"files":[],"notes":[],"message":"could not resolve sources for any entry"}'
```
...
```bash
node .opencode/skills/impeccable/scripts/live-poll.mjs --reply EVENT_ID done --data '{"status":"applied","entries":3}'
```
...
```bash
node .opencode/skills/impeccable/scripts/live-poll.mjs --reply done --file src/page.html
```
...
- To retry an Apply from chat (only when the user explicitly asks and no Track A event is already leased), run `node .opencode/skills/impeccable/scripts/live-commit-manual-edits.mjs --page-url=/<route>` directly. The script prints the same JSON shape as the endpoint and the buffer / rollback semantics are identical.

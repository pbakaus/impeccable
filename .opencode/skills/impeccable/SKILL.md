...
1. Run `node .opencode/skills/impeccable/scripts/context.mjs` once per session. If the user named an exact file, route, or app in a monorepo, you MUST pass it as `--target <path>` in that same command so project-scoped PRODUCT.md / DESIGN.md files can override root defaults. A user asking which PRODUCT.md or DESIGN.md was loaded for a specific path is making a context diagnostic request: run `node .opencode/skills/impeccable/scripts/context.mjs --target <path>` and answer from its `RESOLVED_CONTEXT` fields; do not infer this from the file tree, opened files, or memory. If you accidentally ran `context.mjs` without `--target` for a path-specific monorepo request, that output is not valid evidence; immediately rerun with `--target <path>` before answering. If you've already seen its output for the same target in this conversation, do not re-run it. The script either prints the project's PRODUCT.md (and DESIGN.md when present) as a markdown block, or tells you it's missing. Follow whatever it prints. **If it reports `NO_PRODUCT_MD`, stop and follow `reference/init.md` before doing anything else.** If the output ends with an `UPDATE_AVAILABLE` directive, follow it (ask the user once about updating, then continue). It never blocks the current task.
...
If the user asks which PRODUCT.md or DESIGN.md Impeccable loads for a file, route, or app path, treat that as a diagnostic request, not a design task. Run `node .opencode/skills/impeccable/scripts/context.mjs --target <path>`, report the resolved `productPath`, `designPath`, `projectRoot`, and `repoRoot` from the command's `RESOLVED_CONTEXT` output, and stop. Your final answer must name the command you ran, including `--target <path>`. Do not answer from a directory listing, opened files, or by manually walking ancestor folders; the resolver owns monorepo inheritance and overrides. A correct-looking answer without running the resolver is wrong; if you cannot run the command, say that instead of guessing.

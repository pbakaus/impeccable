---
name: impeccable
description: Use when the user says Impeccable or wants to design, redesign, shape, critique, audit, polish, clarify, distill, harden, optimize, adapt, animate, colorize, extract, or otherwise improve a frontend interface. Covers websites, landing pages, dashboards, product UI, app shells, components, forms, settings, onboarding, and empty states. Handles UX review, visual hierarchy, information architecture, cognitive load, accessibility, performance, responsive behavior, theming, anti-patterns, typography, fonts, spacing, layout, alignment, color, motion, micro-interactions, UX copy, error states, edge cases, i18n, and reusable design systems or tokens. Also use for bland designs that need to become bolder or more delightful, loud designs that should become quieter, live browser iteration on UI elements, ambitious visual effects that should feel technically extraordinary, or context checks that ask which PRODUCT.md / DESIGN.md Impeccable loads for a target file. Not for backend-only or non-UI tasks.
version: 3.7.1
user-invocable: true
...
1. Run `node .opencode/skills/impeccable/scripts/context.mjs` once per session. Use this same scripts directory for all Impeccable helper commands; if a child cwd cannot resolve a relative provider path, walk upward for the nearest project `.agents`, `.claude`, or `.cursor` skill before falling back to a global home install. If it reports `TARGET_SELECTION_REQUIRED`, ask the user which app Impeccable should use and show each option's `PRODUCT.md` and `DESIGN.md` source from `productStatus` / `productPath` and `designStatus` / `designPath`, then rerun helper commands from that child app cwd with this same scripts directory; use `--target <path>` only as a fallback when changing cwd is not possible, or when the user explicitly named a file/path. In a monorepo, path-specific requests still need a real target path. If the user names a route or child app, map it to the matching source file or app directory first, then pass that path so project-scoped PRODUCT.md / DESIGN.md files can override root defaults and missing child files can inherit matching root files. A user asking which PRODUCT.md or DESIGN.md was loaded for a specific path is making a context diagnostic request: run `node .opencode/skills/impeccable/scripts/context.mjs --target <path>` and answer from its `RESOLVED_CONTEXT` fields; do not infer this from the file tree, opened files, or memory. For live requests, inherited root context counts as present; never run init or create child PRODUCT.md / DESIGN.md just because the target child folder lacks them. If you accidentally ran `context.mjs` without `--target` for a path-specific monorepo request, that output is not valid evidence; immediately rerun with `--target <path>` before answering. If you've already seen its output for the same target in this conversation, do not re-run it. The script either prints the project's PRODUCT.md (and DESIGN.md when present) as a markdown block, or tells you it's missing. Follow whatever it prints. **If it reports `NO_PRODUCT_MD`, stop and follow `reference/init.md` before doing anything else.** If the output ends with an `UPDATE_AVAILABLE` directive, follow it (ask the user once about updating, then continue). It never blocks the current task.
...
If the user asks which PRODUCT.md or DESIGN.md Impeccable loads for a file, route, or app path, treat that as a diagnostic request, not a design task. `--target` must be a real file or directory path; map routes and app names to source paths first. Run `node .opencode/skills/impeccable/scripts/context.mjs --target <path>`, report the resolved `productPath`, `designPath`, `projectRoot`, and `repoRoot` from the command's `RESOLVED_CONTEXT` output, and stop. Your final answer must name the command you ran, including `--target <path>`. Do not answer from a directory listing, opened files, or by manually walking ancestor folders; the resolver owns monorepo inheritance and overrides. A correct-looking answer without running the resolver is wrong; if you cannot run the command, say that instead of guessing.
...
| `document` | Build | Generate project DESIGN.md from existing project code | [reference/document.md](reference/document.md) |

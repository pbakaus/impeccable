...
1. Run `node .opencode/skills/impeccable/scripts/context.mjs` once per session. If you know the exact file, route, or app being worked on in a monorepo, pass it as `--target <path>` so project-scoped PRODUCT.md / DESIGN.md files can override root defaults. If you've already seen its output in this conversation, do not re-run it. The script either prints the project's PRODUCT.md (and DESIGN.md when present) as a markdown block, or tells you it's missing. Follow whatever it prints. **If it reports `NO_PRODUCT_MD`, stop and follow `reference/init.md` before doing anything else.** If the output ends with an `UPDATE_AVAILABLE` directive, follow it (ask the user once about updating, then continue). It never blocks the current task.
